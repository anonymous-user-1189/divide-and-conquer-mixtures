######
#Implement MerDel with random clusters selected for merging, time measured for each 'laps' model, and a laps = 0 simulation.
#'mixturemodel_merdelrand'
#####
...
mixturemodel_merdelrand <- function(data, K, alpha, maxiter, tol, laps){
  ################################################
  #Process dataset
  start.time <- Sys.time()
  ...
  for (k in 1:length(laps)){
    ...
    for (iter in 1:maxiter){
      if (lap == "all"){
        ...
        if (iter %% lap == 0){
          model = mergeMoveRand(X, model, prior, secondELBO[ELBOcounter - 1,1])
  ...
  model = expectStep(X, model) #Expectation step
  .GlobalEnv$maxNCat <- maxNCat
  ELBO[1] = secondELBO[ELBOcounter,1] = ELBOCalc(X, model, prior) #ELBO
  secondELBO[ELBOcounter,2] <- Sys.time()
  print(secondELBO[ELBOcounter,1])
  ELBOcounter <- ELBOcounter + 1
  ...
  #M step
  model = maxStep(X, model, prior) #Maximisation step
  ELBO[2] = secondELBO[ELBOcounter,1] = ELBOCalcMStep(X, model, prior) #ELBO
  secondELBO[ELBOcounter,2] <- Sys.time()
  print(secondELBO[ELBOcounter,1])
  ELBOcounter <- ELBOcounter + 1
