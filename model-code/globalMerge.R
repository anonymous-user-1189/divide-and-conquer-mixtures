...
globalMerge <- function(shard_results, prior, Kinit){
  ...
  for (i in 1:(B-1)){ #Go through every shard one by one
    ...
    #update num_clusters as we move to a new shard so we know how many clusters are in the current shard we look at
    ...
    for (j in 1:num_clusters[i]){ #Go through each cluster in each shard
      #Shouldn't update num_clusters because the number of clusters in the current shard i can't change
      
      for (m in (i + 1):B){ #Go through all higher shards for each cluster
        #update num_clusters - there are less clusters in total
        num_clusters <- lapply(shard_results, '[[', 1 )
        num_clusters <- as.vector(unlist(lapply(num_clusters, function(x) length(x)))) #clusters in each shard
        
        
        othershard <- shard_results[[m]]
        ...
        if (k_shard2 == 1){
          ...
          if (i == 1){
            ...
            k1_pos <- k1 + sum(num_clusters[1:(i-1)])
          }
          k2_pos <- k2 + sum(num_clusters[1:(m-1)]) #m is never 1
        ...
        if(k_shard2 > 1){
          currentmodel <- shard_results[[i]] #This has changed as we go through each shard m
          if(num_clusters[i] > 1){
            proposal <- currentmodel$eps[j,1,]
          } else{
            proposal <- currentmodel$eps[1,]
          }
          
          
          all_corrs <- globalcorr_Calc(k_shard2, proposal, othershard$eps, D)
          
          proposal_order <- order(all_corrs, decreasing = TRUE)
          
          for (o in 1:length(proposal_order)){
            #Shouldn't update num_clusters because loop is broken if number of clusters is edited
            k1 <- j
            k2 <- proposal_order[o]
...
globalELBOCalc <- function(mergemodel, prior, Kinit){
  ...
  ELBO <- Exp1 + Exp2 + Exp3 + Exp4 - Exp6 - Exp7
