#Frequency of merge/delete Simulation 1.3 - N=4000, P=100, 10 clusters
...
merdelsim3 <- foreach(k = 1:20) %dorng% {
  generation <- GenerateSampleData(4000, 10, c(0.1, 0.2, 0.1, 0.2, 0.05, 0.05, 0.05, 0.05, 0.1, 0.1), 100, 0)
  data <- generation[[1]]
  ...
  clustlabels <- list()
  for (j in 1:10){
    mix <- mixturemodel_merdelTIME(data, 25, 0.01, 1500, 0.00000005, c(1, 2, 5, 10, 10000))
...
save(merdelsim3, file="merdelsim3.RData")
