#Global merge - variable selection sim, N = 20000
...
globalmergeSimVS1 <- foreach(k = 1:20) %dorng% { 
  
  sampledata <- GenerateSampleData(20000, 15, rep(0.1, 15), 80, 20)
  data <- sampledata[[1]]
  ...
  for (w in 1:5){ #5 different shuffles of the data
    ...
    mainmix <- mixturemodel_merdelTIMEVarSel(data, 30, 0.01, 2, 1000, 0.000005, 5) 
    ...
    indiv_ARI <- c()
    for(i in 1:4){
      mix <- mixturemodel_merdelTIMEVarSel(data_split[[i]], 30, 0.01, 2, 1000, 0.000005, 5)
...
save(globalmergeSimVS1, file = 'globalmergeSimVS1.RData')
