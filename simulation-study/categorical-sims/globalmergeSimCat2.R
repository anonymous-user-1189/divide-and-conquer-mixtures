#Global merge - categorical sim, N = 50000
...
globalmergeSimCat2 <- foreach(k = 1:20) %dorng% { #20 simulated datasets
  
  sampledata <- GenerateSampleDataCat(50000, 10, rep(0.1, 10), 100, 0, yout = FALSE, cat = 3)
  data <- sampledata[[1]]
  ...
  all_results_shuffle1 <- data.frame(run_num = c(1:10), total_time = NA, ARI = NA, clusters = NA, model = rep("shuffle1", 10), merge_time = NA, mode_time = NA, mean_ARI = NA)
  
  for (w in 1:10){ #10 different shuffles of the data
    
    mainmix <- mixturemodel_merdeldiv(data, 20, 0.01, 1000, 0.000005, 5, "KL") 
    ...
    indiv_ARI <- c()
    for(i in 1:5){
      mix <- mixturemodel_merdeldiv(data_split[[i]], 20, 0.01, 1000, 0.000005, 5, "KL")
...
save(globalmergeSimCat2, file = 'globalmergeSimCat2.RData')
