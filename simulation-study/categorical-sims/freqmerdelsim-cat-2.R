#Frequency of merge/delete with categorical data simulations - N=4000, P=100, 10 clusters, 3 categories
...
merdelsim2Cat <- foreach(k = 1:20) %dorng% {
  generation <- GenerateSampleDataCat(4000, 10, c(0.1, 0.2, 0.1, 0.2, 0.05, 0.05, 0.05, 0.05, 0.1, 0.1), 100, 0, yout = FALSE, cat = 3)
  data <- generation[[1]]
  ...
  clustlabels <- list()
  for (j in 1:10){
    mix <- mixturemodel_merdeldiv(data, 25, 0.01, 1500, 0.00000005, c(1, 2, 5, 10, 10000), divergence = "KL")
...
save(merdelsim2Cat, file="/home/jr951/VariationalMixtures/merdelsim2Cat.RData")
