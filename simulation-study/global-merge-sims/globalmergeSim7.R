#Simulations for global merges - N = 50000, split into 10 equal batches of 10000 (third simulation)
...
globalmergeSim7 <- foreach(k = 1:20) %dorng% { #20 simulated datasets
  
  sampledata <- GenerateSampleData(50000, 12, c(0.05, 0.05, 0.05, 0.05, 0.05, 0.1, 0.15, 0.1, 0.1, 0.2, 0.04, 0.06), 100, 0)
  data <- sampledata[[1]]
  ...
  for (w in 1:10){ #10 different shuffles of the data
    ...
    mainmix <- mixturemodel_merdelTIME(data, 20, 0.01, 1000, 0.000005, 5) #Run merge delete model 10 times for good measure
    ...
    indiv_ARI <- c()
    for(i in 1:5){
      mix <- mixturemodel_merdelTIME(data_split[[i]], 20, 0.01, 1000, 0.000005, 5)
...
save(globalmergeSim7, file = 'globalmergeSim7.RData')
