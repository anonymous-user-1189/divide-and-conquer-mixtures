...
all_sims1a <- list()
for (a in 1:15){
  generation <- GenerateSampleData(2000, 10, rep(c(0.1, 0.2, 0.3, 0.2, 0.2), 2), 50, 0)
  data <- generation[[1]]
  ...
  all_results <- foreach(k = 1:10) %dorng%{
    mix1 <- mixturemodel_merdelTIME(data, 20, 0.01, 1500, 0.00000005, c(1, 5))
    mix2 <- mixturemodel_merdeldiv(data, 20, 0.01, 1500, 0.00000005, c(1, 5), divergence = "KL")
    mix3 <- mixturemodel_merdeldiv(data, 20, 0.01, 1500, 0.00000005, c(1, 5), divergence = "bha")
    mix4 <- mixturemodel_merdelrand(data, 20, 0.01, 1500, 0.00000005, c(1, 5))
  ...
  all_sims1a[[a]] <- list(labels, merges, times, clusters)
  
}


save(all_sims1a, file="all_sims1a.RData")
