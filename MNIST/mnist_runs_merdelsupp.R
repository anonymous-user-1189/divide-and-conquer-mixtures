#MNIST global merge and results - K_init = 30 (results in Supplement)
...
full_results <- list()
all_results_shuffle2 <- data.frame(run_num = c(1:10), total_time = NA, ARI = NA, clusters = NA, model = rep("shuffle2", 10))
all_results_shuffle1 <- data.frame(run_num = c(1:10), total_time = NA, ARI = NA, clusters = NA, model = rep("shuffle1", 10)) #1 is seq
...
while (counter < 4){
  ...
  samplemix <- mixturemodel_merdelTIME(mnist_split[[1]], 30, 0.01, 2, 0.000005, 5)
  ...
  shuffled_results <- foreach(k = 1:6)%dorng%{
    mix <- mixturemodel_merdelTIME(mnist_split[[k]], 30, 0.01, 1000, 0.000005, 5)
    mix[[1]]$model
  }
...
mnist_bin_results2 <- list(shuffles, full_results)

save(mnist_bin_results2, file = "mnist_bin_results2.RData")
